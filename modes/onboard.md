...
Also check:
- `profile-skills.md`: if missing, it will be created during onboarding
...
Use exactly the field names in `config/profile.example.yml`, because `scan.mjs` and the matcher read them.
...
- Parse the content in any format: copied Word text, LinkedIn exports, informal summaries
