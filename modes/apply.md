...
- Guarantee/surety questions: some older Nigerian employers ask for a guarantor. Acknowledge you can provide one, but don't give contact details at this stage.
...
- Religion field (some forms still include): if present, answer from profile or leave for user to fill in personally. Don't assume.
...
| NIN | N/A | Only on official government portals at final stage. Never store in files. |
| O'Level (WAEC/NECO/NABTEB) | `education.olevel` | Number of sittings, credits in English & Maths, exam year |
| JAMB/UTME reg. number | N/A | Rare; user fills personally |
...
| Guarantor | N/A | Acknowledge you can provide one; give details only after an offer |
...
- Diversity/equal-opportunity questions (gender, disability, etc.): these are the user's personal choice. List them for the user to fill in, and mention that "Prefer not to say" is always acceptable.
...
```
Application Answers: {Role} at {Company}
